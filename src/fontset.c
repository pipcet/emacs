...
#define RFONT_DEF_SET_FACE(rfont_def, face_id)	\
  ASET ((rfont_def), 0, LRH (make_fixnum (face_id)))
...
/* Score of RFONT_DEF is an integer value; the lowest 8 bits represent
   ...
   preferable.  */
#define RFONT_DEF_SCORE(rfont_def) XFIXNUM (LRH (AREF (rfont_def, 3)))
#define RFONT_DEF_SET_SCORE(rfont_def, score) \
  ASET ((rfont_def), 3, LRH (make_fixnum (score)))
#define RFONT_DEF_NEW(rfont_def, font_def)		\
  do {							\
    (rfont_def) = Fmake_vector (LRH (make_fixnum (4)), LSH (Qnil));	\
...
#define FONTSET_ADD(fontset, range, elt, add)				\
  (NILP (add)								\
   ? (NILP (range)							\
	 ...
	 (fontset, LRH (Fmake_vector (LRH (make_fixnum (1)), (elt)))))  \
      : ((void)								\
	 Fset_char_table_range (fontset, range,				\
				LRH (Fmake_vector (LRH (make_fixnum (1)), elt))))) \
...
{
  return (RFONT_DEF_SCORE (*(ELisp_Struct_Value *)const_cast<void*>(val1))
	  - RFONT_DEF_SCORE (*(ELisp_Struct_Value *)const_cast<void*>(val2)));
}
...
#define SCRIPT_FROM XFIXNUM (LRH (XCAR (LRH (XCAR (script_range_list)))))
#define SCRIPT_TO XFIXNUM (LRH (XCDR (LRH (XCAR (script_range_list)))))
...
{
  ELisp_Pointer realized[2];
  Lisp_Object fontsets[2], tables[2];
  Lisp_Object val, elt;
  int c, i, j, k;
...
syms_of_fontset (void)
{
  ...
  staticpro (&Vfontset_table, Fmake_vector (make_fixnum (32), Qnil));
