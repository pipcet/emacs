  ...
  switch (XFWDTYPE (valcontents))
    ...
    case Lisp_Fwd_Kboard_Obj:
	 ...
	 don't think anything will break.  --lorentey  */
      return *(ELisp_Struct_Value *)(XKBOARD_OBJFWD (valcontents)->offset
...
store_symval_forwarding (union Lisp_Fwd *valcontents, register Lisp_Object newval, struct buffer *buf)
{
  switch (XFWDTYPE (valcontents))
    ...
    case Lisp_Fwd_Obj:
      XOBJFWD (valcontents)->objvar.set(newval);

      /* If this variable is a default for something stored
	 in the buffer itself, such as default-fill-column,
	 find the buffers that don't have local values for it
	 and update them.  */
      if (XOBJFWD (valcontents)->objvar > (void *) &buffer_defaults
	  && XOBJFWD (valcontents)->objvar < (void *) (&buffer_defaults + 1))
	{
	  int offset = ((char *) (XOBJFWD (valcontents)->objvar).u.heap
...
find_symbol_value (Lisp_Object symbol)
{
 ...
 start:
  switch (SYMBOL_REDIRECT (symbol))
    ...
    case SYMBOL_FORWARDED:
      return do_symval_forwarding (SYMBOL_FWD (symbol));
    default: emacs_abort ();
    }
}
              ...
              enum Set_Internal_Bind bindflag)
{
  ...
  CHECK_SYMBOL (symbol);
  switch (SYMBOL_TRAPPED_WRITE_VALUE (symbol))
    {
    case SYMBOL_NOWRITE:
 ...
 start:
  switch (SYMBOL_REDIRECT (symbol))
	...
	union Lisp_Fwd *innercontents = SYMBOL_FWD (symbol);
...
{
  if (SYMBOL_TRAPPED_WRITE_VALUE (symbol) == SYMBOL_NOWRITE)
    xsignal1 (Qtrapping_constant, symbol);
  SET_SYMBOL_TRAPPED_WRITE (symbol, trap);
}
...
harmonize_variable_watchers (Lisp_Object alias, Lisp_Object base_variable)
{
  if (!EQ (base_variable, alias)
    ...
    set_symbol_trapped_write
      (alias, SYMBOL_TRAPPED_WRITE_VALUE (base_variable));
}
      ...
      if (SUBRP (watcher))
        {
          ...
          funcall_subr (XSUBR (watcher), LV (ARRAYELTS (args), args));
        }
      else
...
default_value (Lisp_Object symbol)
{
 ...
 start:
  switch (SYMBOL_REDIRECT (symbol))
    ...
    case SYMBOL_FORWARDED:
      {
	union Lisp_Fwd *valcontents = SYMBOL_FWD (symbol);
 ...
 start:
  switch (SYMBOL_REDIRECT (variable))
	...
	valcontents.value = Qnil;
      break;
    case SYMBOL_LOCALIZED:
      blv = SYMBOL_BLV (variable);
      break;
    case SYMBOL_FORWARDED:
      forwarded = 1; valcontents.fwd = SYMBOL_FWD (variable);
  ...
  (Lisp_Object variable)
{
 ...
 start:
  ...
  if (SYMBOL_TRAPPED_WRITE_VALUE (variable) == SYMBOL_NOWRITE)
    xsignal1 (Qsetting_constant, variable);
      ...
      if (let_shadows_buffer_binding_p (variable))
	{
      ...
      if (BUFFERP (blv->where) && current_buffer == XBUFFER (blv->where))
        /* Make sure the current value is permanently recorded, if it's the
           default value.  */
        swap_in_global_binding (variable);
  ...
  (register Lisp_Object variable)
{
 ...
 start:
  switch (SYMBOL_REDIRECT (variable))
    ...
    case SYMBOL_FORWARDED:
      {
	union Lisp_Fwd *valcontents = SYMBOL_FWD (variable);
	if (BUFFER_OBJFWDP (valcontents))
	  {
	    int offset = XBUFFER_OBJFWD (valcontents)->offset;
	    int idx = PER_BUFFER_IDX (offset);
  ...
  if (SYMBOL_TRAPPED_WRITE_VALUE (variable) == SYMBOL_TRAPPED_WRITE)
 ...
 start:
  switch (SYMBOL_REDIRECT (variable))
    ...
    case SYMBOL_FORWARDED:
      ...
      return (BUFFER_OBJFWDP (SYMBOL_FWD (variable)) ? Qt : Qnil);
    default: emacs_abort ();
    }
}
          ...
          if (sizeof (EMACS_INT) > sizeof (long) && XFIXNUM (num2) > LONG_MAX)
            {
              mpz_t tem;
              mpz_init (tem);
              mpz_set_intmax_slow (tem, XFIXNUM (num2));
          ...
          if (sizeof (EMACS_INT) > sizeof (long) && XFIXNUM (num1) > LONG_MAX)
            {
              mpz_t tem;
              mpz_init (tem);
              mpz_set_intmax_slow (tem, XFIXNUM (num1));
...
#define INTBIG_TO_LISP(i, extremum)				    \
  (eassert (FIXNUM_OVERFLOW_P (i)),				    \
   (! (FIXNUM_OVERFLOW_P ((extremum) >> 16)			    \
    ...
    ? Fcons (LRH (make_fixnum ((i) >> 16)), LRH (make_fixnum ((i) & 0xffff))) \
    : ! (FIXNUM_OVERFLOW_P ((extremum) >> 16 >> 24)		    \
	 && FIXNUM_OVERFLOW_P ((i) >> 16 >> 24))		    \
    ? Fcons (LRH (make_fixnum ((i) >> 16 >> 24)),                   \
	     LRH (Fcons (LRH (make_fixnum ((i) >> 16 & 0xffffff)),  \
                         LRH (make_fixnum ((i) & 0xffffb)))))       \
    : make_float (i)))
  ...
  for (argnum = 0; argnum < nargs; argnum++)
	...
	case Aadd:
	  if (BIGNUMP (val))
	    mpz_add (accum, accum, XBIGNUM (val)->value);
	      ...
	      mpz_t tem;
	      mpz_init (tem);
	      mpz_set_intmax_slow (tem, XFIXNUM (val));
	...
	case Asub:
	  if (! argnum)
		...
		mpz_set_intmax_slow (accum, XFIXNUM (val));
	      if (nargs == 1)
		mpz_neg (accum, accum);
	    }
	  else if (BIGNUMP (val))
  ...
  (register Lisp_Object x, Lisp_Object y)
{
  ...
  if (FIXNUMP (x) && FIXNUMP (y))
	...
	{
	  mpz_init (xm);
	  mpz_set_intmax_slow (xm, XFIXNUM (x));
	  xmp = &xm;
	}
	...
	{
	  mpz_init (ym);
	  mpz_set_intmax_slow (ym, XFIXNUM (y));
	  ymp = &ym;
	}
...
ash_lsh_impl (Lisp_Object value, Lisp_Object count, bool lsh)
{
  ...
  if (BIGNUMP (value))
  ...
  else
    {
      ...
      mpz_set_intmax_slow (result, XFIXNUM (value));
