		      ...
		      re_char *p2)
{
  ...
  op2 = (re_opcode_t) (p2 == pend ? succeed : *p2);
		     ...
		     ptrdiff_t pos, struct re_registers *regs, ptrdiff_t stop)
{
  ...
  for (;;)
	...
	case notwordbound:
	  {
	    bool c_not = (re_opcode_t) *(p - 1) == notwordbound;
	...
	case notsyntaxspec:
	  {
	    bool c_not = (re_opcode_t) *(p - 1) == notsyntaxspec;
	    mcnt = *p++;
	...
	case notcategoryspec:
	  {
	    bool c_not = (re_opcode_t) *(p - 1) == notcategoryspec;
	    mcnt = *p++;
