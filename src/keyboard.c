	...
	if (CONSP (keyhint) && !NILP (XCAR (keyhint)))
	  {
		...
		|| (!EQ (tem, def)
		    ...
		    && !(SYMBOLP (def)
			 && EQ (tem, elisp_symbol_function (def)))))
	      keys = Qnil;
	  }
			...
			bool do_funcall)
{
  ...
  if (SYMBOLP (next) && !NILP (Ffboundp (next))
      && (ARRAYP (elisp_symbol_function (next))
	  || KEYMAPP (elisp_symbol_function (next))))
    next = Fautoload_do_load (elisp_symbol_function (next), next, Qnil);
...
static bool
keyremap_step (Lisp_Object *keybuf, keyremap *fkey,
...
Useful to set before you dump a modified Emacs.  */);
  Vtop_level = Qnil;
  XSYMBOL (Qtop_level)->flags.s.declared_special = false;
